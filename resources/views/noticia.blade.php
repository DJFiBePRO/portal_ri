    ...
    @section('main')
        <main class="page-main">
            <article id="" class="noticia">
                <div class="title">
                    <h1>{{ $news->news_translation_title }}</h1>
                </div>
                <div class="options">
                    @forelse ($multimedia as $contenido)
                        ...
                        @if ($contenido->multimedia_type == 2)
                            <a class="link" href="{{ asset('docs/noticias/' . $contenido->multimedia_name) }}">
                                Documento:
                                {{ $contenido->multimedia_name }} </a>
                        @endif
                        @if ($contenido->multimedia_type == 3)
                            <a class="link" href="{{ $contenido->multimedia_url }}" target="blank"> Enlace:
                                {{ $contenido->multimedia_url }} </a>
                        @endif
                    @empty
